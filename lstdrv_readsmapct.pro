...
pro lstdrv_readSMAPCT,pswfits,pmwfits,plwfits,sra,sdec,prfsize,fcat,f_src=f_src,name=name,astromcorr=astromcorr,fcatmod=fcatmod,noseg=noseg,mralims=mralims,mdeclims=mdeclims,segthresh=segthresh
;
;----------------------------------------------------------------------
...
p_src=fltarr(n_src)
sarea=((max(sra)-min(sra))*cos(mean(sdec)*!pi/180.))*(max(sdec)-min(sdec))
...
fstruct={xid:0L,inra:0.,indec:0.,obs250:0.,chi250:0.,e250:0.,obs350:0.,e350:0.,chi350:0.,obs500:0.,e500:0.,chi500:0.,gid:0L,gsize:0}
...
fcatmod[good].e250=sqrt(fcatmod[good].e250^2+err250[good])
;350
bmask350=fltarr(naxis1_350,naxis2_350)
make_2d,indgen(11)-5,indgen(11)-5,tx,ty
...
fcatmod[good].e350=sqrt(fcatmod[good].e350^2+err350[good])

;500

bmask500=fltarr(naxis1_500,naxis2_500)
make_2d,indgen(11)-5,indgen(11)-6,tx,ty
...
fcatmod[good].e500=sqrt(fcatmod[good].e500^2+err500[good])


; make some residual maps 

rmap250=im250-reconmap.rmap250
rmap350=im350-reconmap.rmap350
rmap500=im500-reconmap.rmap500

;shuffle sources around

testflx250=fcatmod[round(randomu(-6167,n_src)*n_src)].obs250
testflx350=fcatmod[round(randomu(-6167,n_src)*n_src)].obs350
testflx500=fcatmod[round(randomu(-6167,n_src)*n_src)].obs500

cp_src=p_src[round(randomu(-6167,n_src)*n_src)]
mkspiremap,naxis1_250,naxis2_250,sx250+2,sy250-2,testflx250,prf250,tmap250
mkspiremap,naxis1_350,naxis2_350,sx350+2,sy350-2,testflx350,prf350,tmap350
mkspiremap,naxis1_500,naxis2_500,sx500+1,sy500-1,testflx500,prf500,tmap500
tmap250=tmap250+rmap250
tmap350=tmap350+rmap350
tmap500=tmap500+rmap500


fcatcomp=replicate(fstruct,n_src+nnew)
fcatcomp.inra=[sra, nra]
fcatcomp.indec=[sdec,ndec]

segsrccomp=lonarr(n_src)+2
;if not keyword_set(noseg) then 
lstdrv_segsrc,sx250+2,sy250-2,pfwhm[0],pfwhm[1],pfwhm[2],hdr250,tmap250,nim250,hdr350,tmap350,nim350,hdr500,tmap500,nim500,segsrccomp,bsn=segthresh
tmap250=tmap250-bkg250
tmap350=tmap350-bkg350
tmap500=tmap500-bkg500

lstdrv_solvefluxes2,segsrccomp,round(sx250+2),round(sy250-2),round(sx350+2),round(sy350-2),round(sx500+1),round(sy500-1),pfwhm,tmap250,tmap350,tmap500,nim250,nim350,nim500,naxis1_250,naxis2_250,naxis1_350,naxis2_350,naxis1_500,naxis2_500,cp_src,fcat=fcatcomp,reconmap=reconmap,f_src=f_src
stop
end
